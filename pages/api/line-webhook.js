...
export default async function handler(req, res) {
  ...
  req.on('end', async () => {
    try {
      ...
      console.log('Webhook 內容:', JSON.stringify(parsedBody, null, 2));
      ...
      for (const event of events) {
        if (event.type === 'message') {
          ...
          // 處理回覆
          const quotedMessage = event.message.quote?.text || event.message.quotedMessage?.text;
          if (quotedMessage && quotedMessage.startsWith('QA')) {
            // 在查詢時也只使用 QA 後面的文字
            const originalQuestion = quotedMessage.substring(2).trim();
            try {
